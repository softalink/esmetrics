...
//! (mirrors upstream `lib/procutil.WaitForSigterm` plus vmalert/vmauth's
//! SIGHUP config-reload trigger). Ported verbatim from `esmauth::signal`
//! (`crates/esmauth/src/signal.rs`) — the mechanism is process-generic, not
...
//!   atomic flag, which the config-reload watcher thread polls via
//!   [`take_reload_request`]. The shutdown waiter polls the shutdown atomic
//!   via `Condvar::wait_timeout`.
...
//!   Windows has no SIGHUP, so [`take_reload_request`] is always `false` there
//!   and config reload is interval-only.
...
/// reload flag. Returns the human-readable signal name for logging.
...
///
/// Not called by `main`'s wiring (Task 19): esmalert's config reload is
/// driven by `POST /-/reload` and `-configCheckInterval` only (see
/// `app::run_main_loop`); this module is copied from `esmauth::signal`
/// verbatim for its SIGINT/SIGTERM handling, and this function comes along
/// with it.
#[allow(dead_code)]
